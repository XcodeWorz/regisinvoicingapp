$(document).ready(function(){
	...
	var iEditHTML = '<div class="iEdit-img-edit"><div class="modal-dialog"><div class="modal-content"><div class="img-crop"><canvas class="iEdit-img-edit-can"></canvas><canvas class="iEdit-img-edit-process-can"></canvas><div class="iEdit-img-edit-select"><div class="iEdit-img-edit-select-resize"></div></div><div class="iEdit-img-edit-act iEdit-img-edit-cancel"> Cancel </div><div class="iEdit-img-edit-act iEdit-img-edit-save"> Done </div></div></div></div></div>';
	...
	window.iEdit = {
		...
		open: function(imgObj, square, callback, imageType, imageQuality){
			...
			if(imageType == "jpeg" || imageType == "jpg"  || imageType == "png" || imageType == "gif" || imageType == "bmp"){ //JPG and any other would default to JPEG//
			...
			$(img).on("load", function(){
				...
				if(img.width > img.height){
					...
					iEdit.selectionBox.height(300);
					iEdit.selectionBox.width(450);
	...
	$(document).on('mousemove', function(e){
		...
		if(iEdit.drag === true && iEdit.status){
			...
			if(iEdit.square){
				...
				if(e.clientX >= $(iEdit.can).width() + windowOffset($(iEdit.can)).left){
					//REASON: nWidth + windowOffset(iEdit.selectionBox).left = e.clientX;
					nWidth = (windowOffset($(iEdit.can)).left + $(iEdit.can).width()) - (windowOffset(iEdit.selectionBox).left);
					nHeight = (nWidth*2)/3;
					if(nHeight >= e.clientX){
						nHeight = (windowOffset($(iEdit.can)).top + $(iEdit.can).height()) - (windowOffset(iEdit.selectionBox).top);
					}
				}else{
					nWidth = (nHeight*3)/2;
