...
class EstimateDetails extends Component {
    ...
    onDropDownItemClick(item, index){
        switch(index){
            case 0://EDIT
                this.props.history.push("/editestimate/"+this.state.invoices.estimateId);
                break;
            case 1: //CLONE
                this.cloneInvoice(this.state.invoices.estimateId, this.state.invoices.estimateNo);
                break;
            case 2://Download PDF
                this.downloadPdf(this.state.invoices.estimateId,'N')
                break;
            case 3://Mail PDF
                this.mailPdf(this.state.invoices.estimateId, this.state.invoices.estimateNo, this.state.invoices.email, this.state.invoices.firstName)
                break;
            case 4://Convert to invoice
                this.props.history.push("/convertestimateinvoice/"+this.state.invoices.estimateId);
                break;
            case 5://status change
                this.changeStatus(this.state.invoices.estimateId,this.state.invoices.invoice_status)
                break;
    ...
    cloneOperate()
    {
        this.props.history.push('/cloneestimate/'+this.state.cloneInvoiceId);
    }
    
    async mailPdf(invoiceId,invoiceNo,emailTo,emailToName)
    {
        this.openMail();
        this.setState({emailInvoiceId:invoiceId,emailInvoiceNo:invoiceNo,emailTo:emailTo,emailToName:emailToName,emailSubject:"Estimate-"+invoiceNo});
    }
    ...
    async downloadPdf(invoiceId,isEmail)
    {
        ...
        await Promise.all([fetch(Constants.BASE_URL_API+"getestimatedetails",
        {
            method: "POST",
            mode:'cors',
            body: new URLSearchParams({estimateId:invoiceId})
        }),
        fetch(Constants.BASE_URL_API+"getsettings",
        {
            method: "POST",
       ...
       .then(([res1,res2]) => {
         this.setState({
            ...
            pdfInvoiceNo:res1.invoices[0].estimateNo,
            pdfIsIGST:res1.invoices[0].isIGST,
            pdfInvoiceDate:res1.invoices[0].estimateDate,
...
if(isEmail == "N")
{
    pdfMake.createPdf(docDefinition).download('estimate-'+this.state.pdfInvoiceNo+'.pdf');
}
else
{
    var filename = 'estimate-'+this.state.pdfInvoiceNo+'.pdf';
 ...
 document.getElementById("spinnerLoaderDivPdf").style.display = "none";
    ...
    render() {
        ...
        if(this.state.invoices.status === Constants.DRAFT_STATUS)
        {
        ...
        return (
            <Router>
                ...
                <div className="main-panel">
                    <nav className="header navbar">
                        <div className="header-inner">
                            ...
                            <span className="tophead-txt">Estimate >> Details</span>
                                ...
                                <div className="card-block">
                                    ...
                                    <div id="bodyDiv">
                                        <div className="details-header">
                                            <h4>{(this.state.invoices && this.state.invoices!=="undefined" && this.state.invoices!==null)?this.state.invoices.estimateNo:''}</h4>
                                            <div className="details-options">
                                            <a class="btn btn-primary btn-details" href={"/editestimate/"+this.state.invoices.estimateId}>Edit</a>
                                                <DropdownButton
                                                    items={itemsObj} 
                                        ...
                                        <div className="customer-tabs">
                                                <div className="row">
                                                    <div className="col-md-3">
                                                        ...
                                                        <p><b>Estimate No: </b>{(this.state.invoices && this.state.invoices!=="undefined" && this.state.invoices!==null)?this.state.invoices.estimateNo:''}</p>
                                                        <p><b>Estimate Date: </b> {(new Date(this.state.invoices.estimateDate).getDate()) + "-" + (new Date(this.state.invoices.estimateDate).getMonth() + 1) + "-" + (new Date(this.state.invoices.estimateDate).getFullYear())}</p>
                                                        <p>
                                                        <b>Estimate Amount: </b>
                                                        {
                                                        ...
                                                        <p>
                                                            <span className="status-span">
                                                                <b>Status: </b>{this.state.invoices.status}
                                                            </span>    
                                                        </p>
                ...
                <Modal show={this.state.showModal} onHide={this.close}>
                            ...
                            <Modal.Body>
                                <div className="wr-msg">Are you sure you want to clone <br/> Estimate No. {this.state.cloneInvoiceNo}?</div>
                    ...
                    <Modal show={this.state.showModalMail} onHide={this.closeMail}>
                        <Modal.Header closeButton>
                            <Modal.Title>Email Invoice No. {this.state.emailInvoiceNo}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
