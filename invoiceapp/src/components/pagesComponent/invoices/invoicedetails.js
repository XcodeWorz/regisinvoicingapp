...
class InvoiceDetails extends Component {
    constructor(props) {
        ...
        //Check URL Path contains /invoicedetails 
        this.isInvoice = (this.props.location.pathname.includes(Constants.DETAILS_INVOICE_PATH))?true:false;
    ...
    componentDidMount()
    {
        ...
        .then(([res1,res2,res3]) => {
            this.setState(
                {
                    invoices: (res1.invoices && res1.invoices!=="undefined" && res1.invoices !==null)?res1.invoices[0]:{},
                    isRegisteredUnderGST:res2.settings[0].isRegisteredUnderGST,
                    payments: (res3.payments && res3.payments!=="undefined" && res3.payments !==null)?res3.payments:[],
                });
    ...
    onDropDownItemClick(item, index){
        switch(index){
            case 0: //CLONE
                this.cloneInvoice(this.state.invoices.invoiceId, this.state.invoices.invoiceNo);
                break;
            case 1://Download PDF
                this.downloadPdf(this.state.invoices.invoiceId,'N')
                break;
            case 2://Mail PDF
                this.mailPdf(this.state.invoices.invoiceId, this.state.invoices.invoiceNo, this.state.invoices.email, this.state.invoices.firstName)
                break;
            case 3://Change Status
                this.changeStatus(this.state.invoices.invoiceId,this.state.invoices.invoice_status,index)
                break;
    ...
    async mailPdf(invoiceId,invoiceNo,emailTo,emailToName)
    {
         ...
         this.setState({
            ...
            emailSubject:this.isInvoice?'Invoice-'+invoiceNo:''});
    }
    ...
    render() {
        ...
        let itemsObj = [Constants.LISTING_CLONE, Constants.LISTING_DOWNLOAD, Constants.LISTING_EMAIL];
        let itemsIconsObj = [Constants.LISTING_CLONE_ICON, Constants.LISTING_DOWNLOAD_ICON, Constants.LISTING_EMAIL_ICON];
        if(this.state.invoices.invoice_status === Constants.DRAFT_STATUS)
        {
            itemsObj.push(Constants.LISTING_MARK_SENT);
            itemsIconsObj.push(Constants.LISTING_CONVERT_ICON);
        }
        else if(this.state.invoices.invoice_status === Constants.DUE_ON_DATE_STATUS)
        {
            itemsObj.push(Constants.LISTING_MARK_VOID);
            itemsIconsObj.push(Constants.LISTING_CONVERT_ICON);
        }
        else if(this.state.invoices.invoice_status === Constants.CLEARED_STATUS)
        {
            itemsObj.push(Constants.LISTING_MARK_VOID);
            itemsIconsObj.push(Constants.LISTING_CONVERT_ICON);
        }
        ...
        this.state.invoices.invoiceitems && this.state.invoices.invoiceitems!=="undefined" && this.state.invoices.invoiceitems !==null && this.state.invoices.invoiceitems.length > 0 &&
        this.state.invoices.invoiceitems.map((itemholder2, idx2) => {
        if(taxTmpArray.indexOf(itemholder2.tax) > -1)
        {
                ...
                <div className="main-panel">
                    <nav className="header navbar">
                        <div className="header-inner">
                            ...
                            <span className="tophead-txt">{this.isInvoice?"Invoice >> Details":""}</span>
                                ...
                                <div className="card-block">
                                    ...
                                    <div id="bodyDiv">
                                        <div className="details-header">
                                            <h4>{(this.state.invoices && this.state.invoices!=="undefined" && this.state.invoices!==null)?this.state.invoices.invoiceNo:''}</h4>
                                            <div className="details-options">
                                                {
                                                    this.state.invoices && this.state.invoices!=="undefined" && this.state.invoices!==null &&
                                                    <span>
                                                        <a class="btn btn-primary btn-details" href={(this.isInvoice?"/editinvoice/":"#")+this.state.invoices.invoiceId}>Edit</a>
                                                        {   this.state.invoices.invoice_status === Constants.DUE_ON_DATE_STATUS &&
                                                            <a class="btn btn-primary btn-details" href={(this.isInvoice?"/recordpaymentinvoice/":"#")+this.state.invoices.invoiceId}>Record Payment</a>
                                                        }
                                                    </span>
                                                }
                                                <DropdownButton
                                            ...
                                            <Tab eventKey="overview" title="Overview">
                                                <div className="row">
                                                    <div className="col-md-3">
                                                        ...
                                                        <p><b>{this.isInvoice?"Invoice No: ":"" }</b>{(this.state.invoices && this.state.invoices!=="undefined" && this.state.invoices!==null)?this.state.invoices.invoiceNo:''}</p>
                                                        <p><b>{this.isInvoice?"Invoice Date: ":""}</b> {(new Date(this.state.invoices.invoiceDate).getDate()) + "-" + (new Date(this.state.invoices.invoiceDate).getMonth() + 1) + "-" + (new Date(this.state.invoices.invoiceDate).getFullYear())}</p>
                                                        <p>
                                                        <b>{this.isInvoice?"Invoice Amount: ":""}</b>
                                                        <i className="fa fa-inr" aria-hidden="true"></i>
                                                        {
                                                            this.state.invoices.isTaxExclusive ==="N"
                                                            ?
                                                            totalAmount
                                                            :
                                                            (this.state.invoices.invoiceitems && this.state.invoices.invoiceitems!=="undefined" && this.state.invoices.invoiceitems !==null && this.state.invoices.invoiceitems.length > 0)
                                                            ?
                                                            (this.state.invoices.invoiceitems.reduce((sum, i) => (
                                                                sum += i.quantity * i.rate
                                                            ), 0)+(this.state.invoices.invoiceitems.reduce((sum, i) => (
                                                        ...
                                                        <p>
                                                            <span className="status-span">
                                                                <b>Status: </b>{this.state.invoices.invoice_status}
                                                            </span>
                                                        </p>
                                                        <p><b>Create Date: </b>{(new Date(this.state.invoices.createDate).getDate()) + "-" + (new Date(this.state.invoices.createDate).getMonth() + 1) + "-" + (new Date(this.state.invoices.createDate).getFullYear())}</p>
                                                    </div>
